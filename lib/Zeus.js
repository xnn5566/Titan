...
Zeus.getWorkers = function(callback) {
    ...
    for (var appName in AppDatabase){
        for (var id in  AppDatabase[appName].workers){
            ...
            var ts =+ new Date;
            var uptimeDate = new Date(ts - parseInt(worker.lastStartTime));
            var day = uptimeDate.getDate() - 1;
            var hour = uptimeDate.getHours() - 8;
            var minute = uptimeDate.getMinutes();
            workers.push({
                ...
                uptime        : { day : day, hour : hour, minute : minute} // uptime object
            })
        }
    }
